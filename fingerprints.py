...
class fingerprints():
    def __init__(self,nmax=2,lmax=1,r_c=6.0,globalFP=True,descriptor="bispectrum"):
        self.nmax = nmax
        self.lmax = lmax
        self.Rc = r_c
    ...
    def bispectrum(self):
        ...
        array_length = bispect_length(self.nmax,self.lmax)

        grid_size = len(self.rawInput[0]['xyz'])
        num_files = len(self.rawInput)

        self.fingerprint = np.zeros((num_files*grid_size,array_length*2))
        ...
        for i in range(num_files):
            ...
            #glob_bispectrum = get_bispect(self.nmax,self.lmax,self.Rc,get_f90_array(at_posns),get_f90_array(cell),natoms,get_f90_array(grid[0,:]),cg_tensor,array_length,False,array_length)

            for j in range(10):
                bispectrum = get_bispect(self.nmax,self.lmax,self.Rc,get_f90_array(at_posns),get_f90_array(cell),natoms,get_f90_array(grid[j,:]),cg_tensor,array_length,True,array_length)
                bispectrum = np.asarray(bispectrum.T,order='C')
                self.fingerprint[(i+1)*j,:array_length] = bispectrum
                #self.fingerprint[(i+1)*j,array_length:] = glob_bispectrum

            #print(self.fingerprint[1000,:])
